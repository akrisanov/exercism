...
class SetsTest(unittest.TestCase):
    ...
    def test_categorize_dish(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(f"variation #{variant}", item=item, result=result):
                error_message = (
                    f"Exptected category {result} for {item[0]}, but got "
    ...
    def test_compile_ingredients(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(f"variation #{variant}", item=item, result=result):
                error_message = (
                    "Expected a proper set of combined ingredients, "
    ...
    def test_separate_appetizers(self):
        ...
        for variant, (item, result) in enumerate(test_data, start=1):
            with self.subTest(f"variation #{variant}", item=item, result=result):
                error_message = (
                    "Expected only appetizers returned, "
                    "but some dishes remain in the group."
                )
                self.assertEqual(
