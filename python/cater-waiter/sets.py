...
def check_drinks(drink_name: str, drink_ingredients: list[str]) -> str:
    ...
    return f"{drink_name} {'Mocktail' if ALCOHOLS.isdisjoint(drink_ingredients) else 'Cocktail'}"
...
def categorize_dish(dish_name: str, dish_ingredients: list[str]) -> str:
    ...
    (which meal category the dish belongs to). All dishes will "fit" into one of the categories
    imported from `categories.py` (VEGAN, VEGETARIAN, PALEO, KETO, or OMNIVORE).
    """
    for category, ingredients in CATEGORIES.items():
        if set(dish_ingredients) <= ingredients:
...
def tag_special_ingredients(dish: tuple[str, list[str]]) -> tuple[str, set[str]]:
    ...
    on the dish description. For the purposes of this exercise, all allergens or special ingredients
    that need to be tracked are in the SPECIAL_INGREDIENTS constant imported from `categories.py`.
    """

    dish_name = dish[0]
    dish_ingredients = set(dish[1])

    return dish_name, SPECIAL_INGREDIENTS & dish_ingredients
